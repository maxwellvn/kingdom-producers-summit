<?php
/** @var array $summit @var string $reference @var bool $cancelled @var string $error */
?>
<section class="reg">
  <div class="container pay">
    <div class="pay__card">
      <p class="mono pay__kicker"><span class="pay__dot" aria-hidden="true"></span> Payment step</p>
      <h1 class="pay__title">Complete your onsite place</h1>

      <?php if ($cancelled && $error === ''): ?>
      ...
      <?php else: ?>
        <p class="pay__lede">Your details are already with us. Enter the reference and email you registered with to pay the <strong>&pound;<?= number_format(config('stripe.price_pence') / 100, 0) ?></strong> discounted place (standard &pound;<?= number_format(config('stripe.original_pence') / 100, 0) ?>) and confirm your attendance in London.</p>
      <?php endif; ?>

      <form class="pay__form" method="post" action="<?= url('/register/pay') ?>">
        <?= csrf_field() ?>
        <?php if (!empty($savedRegistration)): ?>
        <p>Registration reference: <strong><?= e($reference) ?></strong></p>
