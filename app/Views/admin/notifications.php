          ...
          <?php foreach ($queue as $a): ?>
            <tr>
              ...
              <td><?php if ($a['sent_at'] === null): ?><span class="adm-pill adm-pill--claimed">Scheduled</span>
                  <?php elseif ($a['result'] === 'sending'): $p = \App\Services\Announcer::progress((int) $a['id']); ?><span class="adm-pill adm-pill--claimed">Sending</span><br><span class="adm-muted mono" style="font-size:.8rem"><?= $p['sent'] ?> of <?= $p['total'] ?> sent<?= $p['failed'] ? ', ' . $p['failed'] . ' failed' : '' ?><?= !empty($a['resume_at']) && strtotime($a['resume_at']) > time() ? ' · mail host asked us to wait, resumes ' . e(date('H:i', strtotime($a['resume_at']))) : ' · continues every minute' ?></span>
